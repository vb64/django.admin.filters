...
Поле модели, к которому применяется этот фильтр ('status' в примере выше) должно иметь тип `CharField` или `IntegerField`.
При определении этого поля в `models.py` должен быть использован параметр `choices`.
